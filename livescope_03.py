...
class MainWindow(QtGui.QMainWindow):
    ...
    def on_pushButton_Save_clicked(self):
        if not hasattr(self,'last_trace') : return
        filename = QtGui.QFileDialog.getSaveFileName()
        if filename:
            x,y = self.last_trace
            tosave = {
                'data' : {'time' : x , 'voltage' : y} ,
                'date_time' : time.time() ,
                'acquisition_parameters' : {'scope' : 'Lecroy WaveSurfer'},
                }
            with open(filename, 'w') as f:
                cPickle.dump(tosave, f, cPickle.HIGHEST_PROTOCOL)
